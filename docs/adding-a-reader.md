...
2. For a different payload shape, subclass `BaseGovDataReader[GovDataLocator]` and implement `_parse`; `bundeswahlleiterin.py` and `uba.py` show the pattern. For a different source, also override `locator_type()` and `_fetch`; the Destatis path below shows a POST source with credentials.
