...
| Path | Holds |
| --- | --- |
| `feature_groups/govdata/` | The root `GovDataFeature`, `BaseGovDataReader`, and the GET readers (GovData CKAN, Bundeswahlleiterin `kerg.csv`, UBA Air Data JSON). `core/` is the plumbing every source shares: HTTP client, download cache, CKAN discovery, locator, CSV parsing, provenance. |
...
| `feature_groups/harmonization/` | The chained FeatureGroups on `HarmonizationFeature`: `value__rebased`, `<key>__nuts2024`, `<time>__year_period`. `core/` holds the logic they wrap (keys, NUTS mapping, re-basing, Land codes), `core/reference/` the BBSR, GV-ISys and Eurostat loaders. |
...
5. the harmonization FeatureGroups (their `core/`, the shared cache, the Destatis credential option and
   time-label model)
6. `feature_groups/land_join.py` (the readers and `core/land_codes.py`)
7. the `recipes` package, which imports every FeatureGroup package to register it; the JSON files are data it
   loads by path

A fresh-interpreter test in `feature_groups/destatis/tests/test_reader.py` checks that importing the Destatis
reader loads nothing from entries 4 to 7.
...
- **Add a harmonization:** the logic goes in `feature_groups/harmonization/core/` and returns new rows, never
  ...
  `HarmonizationFeature` subclass in `feature_groups/harmonization/` (copy `nuts.py` over `core/nuts.py`),
  exported from that package's `__init__.py`, since importing it is what registers it. Document it in
...
- **Add a recipe:** [recipes.md](recipes.md). Add its definition to `RECIPES` in `recipes/tests/shipped.py`
  and write the file with `write_recipe`; a test pins every file under the root `recipes/` to its definition.

## Outside the wheel

The wheel ships code only: no package data and no `tests` packages.

- **Reference tables** (BBSR, GV-ISys, Eurostat) are third-party files (dl-de/by-2-0, CC BY 4.0) and large;
  the Eurostat LAU-to-NUTS file alone is about 20 MB. A bundled subset would answer for the keys it holds and
  be silently wrong for every other. Each is fetched once with its loader
  (`load_bbsr_kreise(cache, revalidate=True)` and the like), pinned by URL and sha256, and read offline
  afterwards; the FeatureGroups never fetch and raise with the loader to call on an empty cache. Tests read
