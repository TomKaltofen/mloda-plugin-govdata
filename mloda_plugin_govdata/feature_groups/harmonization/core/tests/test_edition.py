...
from mloda_plugin_govdata.feature_groups.harmonization.core.edition import load_edition
...
def test_load_edition_from_real_fixture(monkeypatch: pytest.MonkeyPatch, reference_fixtures_dir: Path) -> None:
    ...
    monkeypatch.setattr(
        "mloda_plugin_govdata.feature_groups.harmonization.core.edition.load_lau_nuts_de", lambda cache, **kwargs: rows
    )
...
def test_load_edition_raises_on_multiple_periods(monkeypatch: pytest.MonkeyPatch, reference_fixtures_dir: Path) -> None:
    ...
    monkeypatch.setattr(
        "mloda_plugin_govdata.feature_groups.harmonization.core.edition.load_lau_nuts_de", lambda cache, **kwargs: mixed
    )
