...
def _kerg_land_rows(path: Path) -> list[tuple[str, str]]:
...
def test_the_kerg_land_rows_match_the_constant(govdata_fixtures_dir: Path) -> None:
    rows = _kerg_land_rows(govdata_fixtures_dir / "kerg_sample.csv")
    assert len(rows) == 16
...
def test_a_swapped_name_is_reported_by_row() -> None:
    rows = [(code, name) for code, name in LAND_NAMES.items()]
    rows[0] = ("01", "Hamburg")
...
def test_completeness_wants_each_land_exactly_once() -> None:
    fifteen = [(code, name) for code, name in LAND_NAMES.items() if code != "16"]
    ...
    with pytest.raises(LandNameError, match="11 appears 2 times"):
        check_land_names([*LAND_NAMES.items(), ("11", "Berlin")])
